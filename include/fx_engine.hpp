...
class FxEngine {
 public:
  FxEngine(std::span<float> signal)
      : buffer_(signal), mask(buffer_.size() - 1){};
  ~FxEngine() = default;
  ...
  void SetLFOFrequency(LFOIndex index, float frequency) {
    lfos_[index].Init(frequency * 32.0f);
  }
  ...
  float LFO(LFOIndex lfo) {
    ...
    switch (lfo) {
      case LFO_1:
        return lfos_[0].value();
      case LFO_2:
        return lfos_[1].value();
    }
 ...
 private:
  ...
  std::array<CosineOscillator, 2> lfos_;
 ...
 public: /******************** INNER CLASSES ****************/
  class Context {
   public:
    ...
    constexpr void Lp(float& state, float coefficient) {
  ...
  struct DelayLine {
    ...
    float Interpolate(float offset) {
      ...
      return ::Interpolate(a, b, offset_fractional);
    }
  ...
  struct AllPass : public DelayLine {
    ...
    //[gnu::always_inline]
    float Interpolate(Context& c, float offset, LFOIndex index, float amplitude,
                      float scale) {
    ...
    //[gnu::always_inline]
    void ProcessInterpolate(Context& c, float offset, LFOIndex index,
  ...
  static void ConstructTopology(FxEngine& e,
                                std::initializer_list<DelayLine*> delays) {
    size_t base = 0;
